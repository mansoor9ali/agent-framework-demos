...
# Optional: Suppress the "No outgoing edges" warning for terminal nodes
# Remove these lines if you want to see all framework warnings
...
async def main():
    ...
    # Note: The reviewer agent is the terminal node in this workflow, so it will not have
    # any outgoing edges. This is expected behavior. The framework will log a warning
    # "No outgoing edges found for executor reviewer; dropping messages" which can be safely
    # ignored. See step3_streaming.py for how to wrap agents in custom Executor classes
    # to explicitly handle terminal nodes using ctx.yield_output().
