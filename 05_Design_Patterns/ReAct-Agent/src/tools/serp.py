...
def load_api_key() -> str:
    ...
    --------
    str
        The API key from the SERP environment variable.

    Raises:
    -------
    ValueError
        If the SERP environment variable is not set.
    """
    api_key = os.getenv('SERP')
    if not api_key:
        raise ValueError("SERP environment variable is not set. Please set it in your .env file.")
    return api_key
